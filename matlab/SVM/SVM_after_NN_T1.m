function [ mean_result ] = SVM_after_NN_T1( task, KernelSVM, List_of_subject, nn, Size_of_feat, fast, channel_type)
...
for subject_i=1:size(List_of_subject,2)
    ...
    for f_valid = 1 : nbFolds
        ...
        Indexes={};
        trainTrials_fold = dat.T1.train_sample{f_valid};
        ...
        %% save
        if fast
            outputDir = strcat('Data/NN_results/',channel_type,'/',nn,'/task',num2str(task),'/fast/T1/');
        else
            outputDir = strcat('Data/NN_results/',channel_type,'/',nn,'/task',num2str(task),'/slow/T1/');
        end
