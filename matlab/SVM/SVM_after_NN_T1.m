...
for subject_i=1:size(List_of_subject,2)
    ...
    Indexes={};
    nbFolds = size(dat.T1.train_sample,1);
    
    
    %% learn for first feature
    accuracy=zeros(1,size(dat.T1.train_sample{1},2));
    for Nchanel=1:size(dat.T1.train_sample{1},2)
        % Cross validate nbFolds
        ACC=zeros(1,nbFolds);
        for f = 1 : nbFolds

            trainTrials = dat.T1.train_sample{f}(:,Nchanel);
            testTrials = dat.T1.test_sample{f}(:,Nchanel);
    ...
    for Nofeat=2:Size_of_feat
        ...
        %check all features
        accuracy=zeros(size(groups,2),size(dat.T1.train_sample{1},2));
        for Nchanel=1:size(dat.T1.train_sample{1},2)
            for Ngroup=1:size(groups,2)
                if(find(groups{Ngroup}==Nchanel))
                    ...
                    ACC=zeros(1,nbFolds);
                    for f = 1 : nbFolds
                        trainTrials = dat.T1.train_sample{f}(:,FIdx);
                        testTrials = dat.T1.test_sample{f}(:,FIdx);
                        testCues = dat.test_y{f}(:,1);
                        trainCues = dat.train_y{f}(:,1);
    ...
    %% save
    if fast
        outputDir = strcat('Data/SVM_results/nn',num2str(nn),'/task',num2str(task),'/fast/T1/');
    else
        outputDir = strcat('Data/SVM_results/nn',num2str(nn),'/task',num2str(task),'/slow/T1/');
    end
