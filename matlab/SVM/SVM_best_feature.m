...
for subject_i=1:size(List_of_subject,2)
    ...
    for i=1:length(groupT1)
        feat=fix((groupT1(i)-1)/2)+1;
        best_features_T1(feat,i)=best_features_T1(feat,i)+1;
    end
    for i=1:length(groupT2)
        feat=fix((groupT2(i)-1)/2)+1;
...
res_featT1=res_featT1_all(1:10)*2;
res_featT2=res_featT2_all(1:10)*2;

data=loadjson(strcat('Data/SVM/nn',num2str(nn),'/data_for_svm_',List_of_subject{1},'.json'));
for i=1:length(res_featT1)
    labels_best_T1{i}=data.result_label{res_featT1(i)};
    labels_best_T2{i}=data.result_label{res_featT2(i)};
end
if fast
    outputDir = strcat('Data/SVM_final/nn',num2str(nn),'/task',num2str(task),'/fast/');
else
    outputDir = strcat('Data/SVM_final/nn',num2str(nn),'/task',num2str(task),'/slow/');
end
