function [ mean_result ] = PCA_SVM_T2_new( task, Size_of_feat, KernelSVM, ...
...
for subject_i=1:size_of_sub
    ...
    nbFolds = 5;
    uniqueTrials = (1:size(Subject.T2,1));
    ...
    for f_valid = 1 : nbFolds
        ...
        if fast
            if knn
                outputDir = strcat('Data/PCA_results/task',num2str(task),'/',name_folder,'/fast/T2/knn',num2str(knn),'/',KernelSVM,'/');
            else
                outputDir = strcat('Data/PCA_results/task',num2str(task),'/',name_folder,'/fast/T2/',KernelSVM,'/');
            end
        else
            if knn
                outputDir = strcat('Data/PCA_results/task',num2str(task),'/',name_folder,'/slow/T2/knn',num2str(knn),'/',KernelSVM,'/');
            else
                outputDir = strcat('Data/PCA_results/task',num2str(task),'/',name_folder,'/slow/T2/',KernelSVM,'/');
            end
        end
