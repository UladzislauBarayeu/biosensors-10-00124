...
def train_both_tasks(nn, s, number_of_folds=5, number_for_test=10 ,epoch=160, period=2, lr=0.0001, two_times=False, batch_size=36, with_test=False):
    ...
    test1 = EEGdata()
    file = str(s) + '.json'
    test1.load_raw_data(file, directory='Task1/', task='T1', load_false_data_from_files=True,
                        data_len=number_for_test * number_of_folds)

    test2 = EEGdata()
    file = str(s) + '.json'
    test2.load_raw_data(file, directory='Task1/', task='T2', load_false_data_from_files=False, other=test1,
                        data_len=number_for_test * number_of_folds)
    ...
    file_raw = 'two-task-nn/nn' + str(nn) + '/test_conv_ae.h5'
...
#train_both_tasks(21, 85, two_times=True, batch_size=36, with_test=True, lr=0.001, epoch=180)
