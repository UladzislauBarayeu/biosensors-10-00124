...
def load_network(h5file, name=None):
    '''
    load network from h5file, give it name(optional)
...
def train_autoencoder(h5file, train_data,labels, batch_size=32, callbacks_list=None, optimizer=None,
                      ...
                      loss='mean_squared_error', period=50, validation_split=0.1, aepath='output-nn', earlystop=False):
    ...
    if callbacks_list is None:
        ...
        earlycallback = keras.callbacks.EarlyStopping(monitor='loss',mode='auto', min_delta=0.00000001, patience=1, verbose=1,)
    ...
    model_info = network.fit(train_data, labels, epochs=epoch, shuffle=True,
                                 batch_size=batch_size, validation_split=validation_split, verbose=2, callbacks=callbacks_list)
    # if save_ae:
